"""Windows backend tests — mirror of test_executor.py for the Job Object path.

These are skipped on every non-Windows platform (so macOS and Linux CI stay
green) and additionally skipped if the runtime venv hasn't been built.
"""
...
def test_env_is_minimal(workdir: Path) -> None:
    ...
    # No leaked host identity; jail essentials are present.
    assert "USERNAME" not in keys
    assert "HOME" in keys
    assert "PATH" in keys


def test_fork_bomb_is_capped(workdir: Path) -> None:
    """ActiveProcessLimit=1 must prevent the child from spawning descendants."""
    code = "import subprocess, sys; subprocess.Popen([sys.executable, '-c', 'pass'])"
    result = sb.execute_python(code, _policy(workdir))
    # Spawning a second process in the job must fail (non-zero exit), not succeed.
    assert result.exit_code != 0
